...
void Disp2Dec(uint32_t Dec_num)
{
    if(Dec_num == 0){
        XmitUART2(' ',1);  // Disp Gap
        XmitUART2('0',1);  // Disp Gap
        XmitUART2(' ',1);  // Disp Gap
        return;
    }
    uint32_t rem;  //remainder in div by 10
    uint32_t quot; 
    uint32_t ctr = 0;  //counter
    XmitUART2(' ',1);  // Disp Gap
    uint32_t digits = log10(Dec_num) + 1;
    while(ctr<digits)
    {
        quot = Dec_num/(pow(10,(digits-1-ctr)));
        rem = quot%10;
...
// Displays float in decimal form
void Disp2Float(float f)
{
    f = f * 100;
    uint16_t int_num = (uint16_t)f;
    uint8_t rem;  //remainder in div by 10
    uint16_t quot; 
    uint8_t ctr = 0;  //counter
    XmitUART2(' ',1);  // Disp Gap
    
    quot = int_num/(pow(10,(2-ctr)));
    rem = quot%10;
    XmitUART2(rem + 0x30 , 1);
    ctr = ctr + 1;
